...
if not stats_df.empty:
    ...
    # Filter using calculated real bounds
    ...
    if not top_5_df.empty:
        for _, row in top_5_df.iterrows():
            ...
            # Relative score
            pct = ((row['rate_sqm'] - stats['real_min']) / (stats['real_max'] - stats['real_min'])) * 100
            c4.metric("Value Score", f"Top {pct:.1f}%")
            st.divider()
    else:
    ...
    st.info("No data collected yet. Click '🚀 Run Scraper & Market Engine' above to process Morningside.")
